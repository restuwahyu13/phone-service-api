...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.withSchema('public').createTable('device', (table: Knex.TableBuilder) => {
    ...
    table.integer('company_id').references('id').inTable('company').onDelete('CASCADE').unsigned().notNullable().index()
    table.string('device_cd', 50).notNullable()
    table.string('description', 255).notNullable()
    table.boolean('active').notNullable()
    table.enum('complexity', ['low', 'medium', 'high'])
    table.integer('created_by_id').references('id').inTable('user').unsigned().notNullable().index()
    ...
    table.integer('noted_id').unsigned().nullable()
