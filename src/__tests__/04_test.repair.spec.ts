...
describe('Repair Group Testing', function () {
  ...
  it('Should be create repair success', async function () {
    const res = await chai.request(app).post('/api/v1/repair').set('conten-type', 'application/json').send(repairPayload)
  ...
  it('Should be create repair error', async function () {
    const res = await chai.request(app).post('/api/v1/repair').set('conten-type', 'application/json').send({})
  ...
  it('Should be results all repair error', async function () {
    const res = await chai.request(app).get('/api/v1/repair').set('conten-type', 'application/json')

    expect(res.status).to.be.equal(400)
  })

  it('Should be result by id repair success', async function () {
    const res = await chai.request(app).get(`/api/v1/repair/${2}`).set('conten-type', 'application/json')
  ...
  it('Should be result by id repair error', async function () {
    const res = await chai.request(app).get('/api/v1/repair/abc').set('conten-type', 'application/json')

    expect(res.status).to.be.equal(400)
  })

  it('Should be update repair success', async function () {
    const res = await chai.request(app).put(`/api/v1/repair/${2}`).set('conten-type', 'application/json').send(repairPayload)
  ...
  it('Should be update repair error', async function () {
    const res = await chai.request(app).put(`/api/v1/repair/${2}`).set('conten-type', 'application/json').send({})

    expect(res.status).to.be.equal(400)
  })

  it('Should be delete by id repair success', async function () {
    const res = await chai.request(app).delete(`/api/v1/repair/${2}`).set('conten-type', 'application/json')
